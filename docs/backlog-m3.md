...
## TUI 后续（plan-tui 未尽事项）

13. **主题配色系统**：现状硬编码 ANSI 色；未来支持 light/terminal 主题探测与自定义。
14. **鼠标滚动支持**：消息历史目前靠终端原生缓冲区滚动；Ink 上游支持后可做区块滚动。
15. **onboarding 视觉重设计**：与 TUI 新风格对齐（选中态、说明文案）。
16. **装配期进度可见**：MCP 连接/repo map 阶段性回报到忙碌行（当前只有"正在装配上下文…"）。
