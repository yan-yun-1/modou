...
| C4 Zed 实测 | ✅ 协议层全通 + UI 实测通过（一问一答 + 审批创建文件）；联调中修复 answerById 竞态挂死：approve() 先入队再 yield approval_request（+回归测试） | 2026-09-26 |
...
| E5 v0.5.0-alpha 发布 | ✅ GitHub Release 已发；npm 发布 @modou-dev/core/sdk/server（alpha tag）。首次发布的 sdk/server@0.5.0-alpha 依赖含 workspace 协议无法安装，已重发 0.5.0-alpha.1 并置 latest、旧版 deprecated；第三方安装冒烟通过 | 2026-09-26 |
